      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12 space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {collections.slice(0, 2).map((col, idx) => {
            const isAlt = idx % 2 === 1;
            const bgClass = isAlt
              ? 'bg-gradient-to-br from-[#180F0A] via-[#241812] to-[#3E3027] border-[#C5983A]/30'
              : 'bg-[#241812] border-[#3E3027]';
            ...
            return (
              <div
                key={col.id}
                className={`${bgClass} text-[#FAF7F2] rounded-3xl p-8 sm:p-12 border shadow-[0_16px_36px_rgba(36,24,18,0.14)] flex flex-col justify-between min-h-[390px] relative overflow-hidden group`}
              >
                ...
                <div className="pt-6 relative z-10 flex items-center justify-between border-t border-[#3E3027]">
                  <span className="text-xs font-bold text-[#DEB554]">Curated Selection</span>
                  <Button
                    variant="gold"
                    size="md"

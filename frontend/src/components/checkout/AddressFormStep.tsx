  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 font-sans">
      ...
      {savedAddresses.length > 0 && (
        <div className="space-y-3 p-4 bg-[#F4EFE6] border border-[#D4AF37]/50 rounded-md">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="text-xs font-bold text-[#B88B38] hover:underline cursor-pointer flex items-center gap-1"
            >
              ➕ Add New Address
            </button>
          </div>
              ...
              <div
                ...
                className={`p-3 rounded-md border text-xs cursor-pointer transition-all ${
                  selectedAddressIdx === idx
                    ? 'border-[#D4AF37] bg-[#FAF6F0] shadow-card ring-1 ring-[#D4AF37]'
                    : 'border-[#E5D9C5] bg-[#FAF6F0]/60 hover:border-[#D4AF37]'
                }`}
              >
                <div className="flex items-center justify-between font-bold text-[#2A1E17] mb-1">
                  <span>{addr.firstName} {addr.lastName}</span>
                  {selectedAddressIdx === idx && (
                    <span className="text-[10px] bg-[#D4AF37] text-white px-1.5 py-0.5 rounded-xs font-bold">✓ Selected</span>
                  )}
                </div>
                <p className="text-[#69574A] truncate font-medium">{addr.street}</p>
                <p className="text-[#8C7A6B]">{addr.city}, {addr.state} {addr.zip}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <label className="text-xs font-semibold text-[#2A1E17]">State / Region *</label>
              <button
                type="button"
                onClick={() => setIsCustomState(!isCustomState)}
                className="text-[10px] font-bold text-[#B88B38] hover:underline cursor-pointer"
              >
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <label className="text-xs font-semibold text-[#2A1E17]">City *</label>
              <button
                type="button"

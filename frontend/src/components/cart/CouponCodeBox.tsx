...
export const CouponCodeBox: React.FC<CouponCodeBoxProps> = ({
  ...
  // Only show coupons added via Admin panel — no hardcoded defaults
  const activeCouponsList = (coupons || []).map((c) => ({
    code: c.code,
            ...
            <div className="p-3 bg-[#FAF7F2] border border-[#EADDCB]/50 rounded-xl space-y-2 text-xs">
              ...
              <div className="space-y-2">
                {activeCouponsList.length === 0 ? (
                  <p className="text-[11px] text-[#7D6F63] text-center py-2">No active offers available right now.</p>
                ) : (
                  activeCouponsList.map((cpn) => (
                    <div
                      key={cpn.code}
